...
# Assuming the PEER module and Triton functions are accessible
# Adjust imports based on your project structure
from llm.models.experts import PEER
from llm.ops.triton_peer_kernels import peer_selection_triton, HAS_TRITON
...
def get_expert_indices_pytorch(
    ...
    num_experts = np.prod(product_key_dim)
    ...
    for dim_idx in range(num_dims):
        indices_for_dim = flat_mesh_indices[:, dim_idx].unsqueeze(0).unsqueeze(0).unsqueeze(0)
...
test_configs = [
    ...
    pytest.param({
        "id": "default_fp64", "dtype": torch.double, "num_experts": 64, "top_k": 4,
        "num_heads": 2, "query_dim": 32, "product_key_dim": [8, 8],
        "norm_keys": True, "norm_query": True
    }, marks=pytest.mark.skipif(not HAS_TRITON, reason="Triton not installed")),
    # Float32 config
    pytest.param({
        "id": "default_fp32", "dtype": torch.float32, "num_experts": 64, "top_k": 4,
        "num_heads": 2, "query_dim": 32, "product_key_dim": [8, 8],
        "norm_keys": True, "norm_query": True
    }, marks=pytest.mark.skipif(not HAS_TRITON, reason="Triton not installed")),
    # Float16 config (gradcheck might be less stable)
    pytest.param({
        "id": "default_fp16", "dtype": torch.float16, "num_experts": 64, "top_k": 4,
        "num_heads": 2, "query_dim": 32, "product_key_dim": [8, 8],
        "norm_keys": True, "norm_query": True
    }, marks=pytest.mark.skipif(not HAS_TRITON, reason="Triton not installed")),
    # Edge case: top_k = 1
    pytest.param({
        "id": "topk1_fp32", "dtype": torch.float32, "num_experts": 64, "top_k": 1,
        "num_heads": 2, "query_dim": 32, "product_key_dim": [8, 8],
        "norm_keys": True, "norm_query": True
    }, marks=pytest.mark.skipif(not HAS_TRITON, reason="Triton not installed")),
    # Edge case: 3D product key
    pytest.param({
        "id": "3d_fp32", "dtype": torch.float32, "num_experts": 64, "top_k": 4,
        "num_heads": 2, "query_dim": 30, "product_key_dim": [4, 4, 4], # query_dim must be divisible by num_dims
        "norm_keys": True, "norm_query": True
    }, marks=pytest.mark.skipif(not HAS_TRITON, reason="Triton not installed")),
    # No normalization
    pytest.param({
        "id": "nonorm_fp32", "dtype": torch.float32, "num_experts": 64, "top_k": 4,
        "num_heads": 2, "query_dim": 32, "product_key_dim": [8, 8],
        "norm_keys": False, "norm_query": False
    }, marks=pytest.mark.skipif(not HAS_TRITON, reason="Triton not installed")),
]

@pytest.fixture(scope="module", params=test_configs, ids=[c.values[0]["id"] for c in test_configs])
...
def sample_inputs(config_params):
    ...
    if device.type == 'cpu':
         pytest.skip("Skipping Triton tests on CPU") # Skip if no CUDA

    queries = torch.randn(batch_size, seq_len, num_heads, query_dim, device=device, dtype=dtype, requires_grad=True)
    ...
    for dim_size in config_params["product_key_dim"]:
        ...
        keys = torch.randn(dim_size, sub_query_dim, device=device, dtype=dtype, requires_grad=True)
...
def test_forward_correctness(config_params, sample_inputs):
    ...
    assert torch.equal(ref_indices.to(triton_indices.dtype), triton_indices), \
    ...
    assert torch.allclose(ref_scores.to(triton_scores.dtype), triton_scores, atol=atol, rtol=rtol), \
...
def test_backward_gradients(config_params, sample_inputs):
    ...
    if dtype != torch.double:
         pytest.skip("Gradcheck requires double precision. Skipping for non-fp64 configs.")

    # Ensure inputs require grad
    queries.requires_grad_(True)
    sub_keys_tensors = [sk.detach().clone().requires_grad_(True) for sk in sub_keys_list]

    # Use gradcheck
    def gradcheck_func(q, *sks):
        sk_list = list(sks)
        # Run the function under test
        _, scores = peer_selection_triton(q, sk_list, product_key_dim, top_k, norm_query, norm_keys)
    ...
    try:
        ...
        is_correct = gradcheck(gradcheck_func, inputs_for_gradcheck, eps=1e-6, atol=1e-4, nondet_tol=1e-5, raise_exception=True)
        assert is_correct, f"Gradient check failed for PEERSelectionFunction with config: {config_params['id']}"
    except RuntimeError as e:
        pytest.fail(f"Gradcheck failed for config {config_params['id']} with error: {e}")
