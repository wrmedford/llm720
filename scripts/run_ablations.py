...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run ablation studies for PEER architecture"
    )
    parser.add_argument(
        "--base-config",
        type=str,
    ...
    parser.add_argument(
        "--gpus", type=int, default=4, help="Number of GPUs to use for each experiment"
    )
    parser.add_argument(
        "--tokens",
        type=int,
    ...
    parser.add_argument(
        "--resume", action="store_true", help="Resume from a previous ablation study"
    )
    parser.add_argument(
        "--custom-datasets",
        type=str,
