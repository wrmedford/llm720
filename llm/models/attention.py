...
"""
Multi-Headed Latent Attention (MLA) implementation using FlashAttention.

Adapts the DeepSeek MLA architecture for optimized execution.
"""
...
def apply_rotary_pos_emb(
    ...
    Args:
        t (torch.Tensor): Input tensor of shape (batch_size, seq_len, num_heads, head_dim)
                          or (total_tokens, num_heads, head_dim).
        cos (torch.Tensor): Cosine components cache of shape (max_seq_len, rotary_dim // 2).
        sin (torch.Tensor): Sine components cache of shape (max_seq_len, rotary_dim // 2).
        position_ids (torch.LongTensor): Tensor containing the position indices for `t`.
    ...
    # Reshape position_ids to match the sequence dimension of t if necessary
    # t might be (bsz, seq_len, num_heads, head_dim) or (total_tokens, num_heads, head_dim)
    # position_ids might be (bsz, seq_len) or (total_tokens,)
    if position_ids.dim() == 2 and t.dim() == 4: # (bsz, seq_len, ...)
        seq_dim = 1
    elif position_ids.dim() == 1 and t.dim() == 3: # (total_tokens, ...)
        seq_dim = 0
    else:
         # Handle potential mismatch or flatten if needed
         position_ids = position_ids.reshape(-1) # Flatten position_ids
         t = t.reshape(-1, t.shape[-2], t.shape[-1]) # Flatten t's batch/seq dims
         seq_dim = 0
         if position_ids.shape[0] != t.shape[0]:
              raise ValueError(f"Shape mismatch between flattened input ({t.shape}) and position_ids ({position_ids.shape})")


    # Gather the cosine and sine embeddings based on position IDs
    # cos/sin shape: [max_seq_len, rotary_dim // 2]
    # position_ids shape: [num_tokens]
    # gathered cos/sin shape: [num_tokens, rotary_dim // 2]
    cos = cos[position_ids]
    sin = sin[position_ids]

    # Expand cos/sin to match the number of heads: [num_tokens, 1, rotary_dim // 2]
...
class MultiHeadedLatentAttention(nn.Module):
    ...
    def __init__(
        ...
        # self.layer_idx = layer_idx # Removed layer_idx
        self.num_heads = num_heads
        self.q_lora_rank = q_lora_rank
        self.kv_lora_rank = kv_lora_rank
        self.qk_rope_head_dim = qk_rope_head_dim
        self.qk_nope_head_dim = qk_nope_head_dim
        self.q_head_dim = qk_nope_head_dim + qk_rope_head_dim # Combined QK head dim
        ...
        self.num_kv_heads = 1 # MLA uses num_kv_heads = 1 conceptually for latent cache

        # Initialize query projections
        if self.q_lora_rank is None:
            # Standard Q projection (replace with TE Linear)
            self.q_proj = te.Linear(hidden_size, num_heads * self.q_head_dim, bias=False)
        else:
            # Q projection with LoRA (replace with TE Linear/LayerNorm)
            self.q_a_proj = te.Linear(hidden_size, q_lora_rank, bias=False)
            self.q_a_layernorm = te.LayerNorm(q_lora_rank, eps=1e-6) # Assuming eps=1e-6, adjust if needed
            self.q_b_proj = te.Linear(q_lora_rank, num_heads * self.q_head_dim, bias=False)

        # Key-value projections (MLA specific) (replace with TE Linear/LayerNorm)
        ...
        self.kv_a_layernorm = te.LayerNorm(kv_lora_rank, eps=1e-6) # Assuming eps=1e-6
        ...
        # These need to be updated if kv_b_proj weights change (e.g., after loading checkpoint)
        self.W_UV = None
        self.W_UK_T = None
    ...
    def _prepare_kv_weights(self):
        """
        Precomputes transposed/reshaped weights from kv_b_proj for MQA decode path.
        Call this after model initialization and after loading weights.
        """
        ...
        try:
            ...
            # kv_b_proj weight shape: [num_heads * (qk_nope + v_dim), kv_lora_rank]
            ...
            # Here W_UK is (N, P, L), so we need transpose(1, 2) -> (N, L, P)? No, bmm expects (N, B, P) x (N, P, L)
            # Let's stick to VLLM's naming convention for clarity W_UK_T = W_UK.transpose(1, 2)
            # q_nope (N, B, P) @ W_UK_T (N, P, L) -> ql_nope (N, B, L)
            self.W_UK_T = W_UK.contiguous() # Shape [num_heads, qk_nope_head_dim, kv_lora_rank]

            print("Successfully prepared W_UV and W_UK_T for MLA decode.")
        except Exception as e:
            print(f"Warning: Failed to prepare W_UV and W_UK_T: {e}. Decode path might fail.")
            self.W_UV = None
            self.W_UK_T = None
    ...
    # Add a method to explicitly call _prepare_kv_weights, e.g., after loading state_dict
    def post_weight_load(self):
        """Call this method after loading weights to prepare decode matrices."""
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None, # Can be None or 2D/4D mask
        position_ids: Optional[torch.LongTensor] = None, # Crucial for RoPE
        past_key_value: Optional[Tuple[torch.Tensor, torch.Tensor]] = None, # Cache is (kv_c, k_pe)
        output_attentions: bool = False, # Optimized kernels often don't return weights
        ...
        **kwargs, # Allow for extra arguments if needed
        ...
        Args:
            hidden_states (`torch.Tensor`): input of shape `(batch, seq_len, embed_dim)`
            cos (`torch.Tensor`): Cosine components for RoPE of shape `(max_seq_len, rope_dim // 2)`
            sin (`torch.Tensor`): Sine components for RoPE of shape `(max_seq_len, rope_dim // 2)`
            attention_mask (`torch.Tensor`, *optional*): Mask for attention. FlashAttention typically
                infers causal mask if None. If provided, needs compatible shape.
            position_ids (`torch.LongTensor`, *optional*): Positions for RoPE application. Shape `(batch, seq_len)`.
            past_key_value (`Tuple(torch.Tensor, torch.Tensor)`, *optional*): Cached latent states `(past_kv_c, past_k_pe)`.
                `past_kv_c` shape: `(batch, num_heads, past_seq_len, kv_lora_rank)`
                `past_k_pe` shape: `(batch, num_heads, past_seq_len, qk_rope_head_dim)`
            output_attentions (`bool`, *optional*): Whether to return attention weights (not supported by FlashAttention).
            use_cache (`bool`, *optional*): Whether to use and return KV cache.

        Returns:
            Tuple containing:
                - attn_output (`torch.Tensor`): Output tensor of shape `(batch, seq_len, hidden_size)`
                - attn_weights (`Optional[torch.Tensor]`): Always None for FlashAttention.
                - present_key_value (`Optional[Tuple[torch.Tensor, torch.Tensor]]`): Updated cache `(kv_c_cache, k_pe_cache)`.
        """
        if output_attentions:
            # logger.warning("FlashAttention does not support outputting attention weights.")
            print("Warning: FlashAttention does not support outputting attention weights.")
        ...
        if self.q_lora_rank is None:
            ...
            q_proj_output = self.q_proj(hidden_states) # TE Linear returns tensor
        else:
        ...
        compressed_kv = self.kv_a_proj_with_mqa(hidden_states) # TE Linear returns tensor
        ...
        if past_key_value is not None:
            if isinstance(past_key_value, tuple) and len(past_key_value) == 2:
                ...
                past_len = past_kv_c.shape[1]
            else:
                print("Warning: Received past_key_value in unexpected format for MLA. Ignoring cache.")
                past_key_value = None # Treat as no cache provided

        # --- RoPE Application (Applied only to current tokens) ---
        # --- RoPE Application ---
        # Apply RoPE to the PE part of the *current* query
        # q_pe shape: [bsz, q_len, num_heads, qk_rope_head_dim]
        # position_ids shape: [bsz, q_len] -> contains absolute positions for current tokens
        ...
        k_pe = k_pe.squeeze(2) # Back to [bsz, q_len, qk_rope_head_dim]
        ...
        # Reshape query_states for FA: [bsz * q_len, num_heads, q_head_dim] or [total_tokens, num_heads, q_head_dim]
        ...
        attn_output_proj_input = None # Will hold the input to the final o_proj

        # Need cu_seqlens_q and cu_seqlens_k for flash_attn_varlen_func if bsz > 1
        # For standard training loop with batching, assume all sequences in batch have same length for now
        # If using packing/variable lengths, this needs adjustment.
        # cu_seqlens_q: Cumulative sequence lengths for queries in the batch.
        # cu_seqlens_k: Cumulative sequence lengths for keys/values in the batch.
        ...
        if HAS_FLASH_ATTN:
            ...
            if is_decode:
                ...
                # Project q_nope using W_UK_T to get ql_nope [bsz, q_len, num_heads, kv_lora_rank]
                if self.W_UK_T is None or self.W_UV is None:
                    self._prepare_kv_weights() # Try again if weights were loaded after init
                    if self.W_UK_T is None or self.W_UV is None:
                         raise RuntimeError("kv_b_proj weights (W_UK_T, W_UV) not prepared for decode.")
                ...
                q_nope_reshaped = q_nope.permute(0, 2, 1, 3) # [bsz, num_heads, q_len, qk_nope]
                ql_nope = torch.matmul(q_nope_reshaped, self.W_UK_T) # [bsz, num_heads, q_len, kv_lora_rank]

                # q_pe_rotated: [bsz, q_len, num_heads, qk_rope_head_dim] -> [bsz, num_heads, q_len, qk_rope_head_dim]
                q_pe_rotated_reshaped = q_pe_rotated.permute(0, 2, 1, 3)

                # Combine ql_nope and q_pe_rotated for MQA query
                # q_mqa: [bsz, num_heads, q_len, kv_lora_rank + qk_rope_head_dim]
                q_mqa = torch.cat([ql_nope, q_pe_rotated_reshaped], dim=-1)
                q_mqa = q_mqa.reshape(-1, self.num_heads, self.kv_lora_rank + self.qk_rope_head_dim) # [total_q_tokens, num_heads, head_dim_mqa]
                ...
                # Reshape for FA MQA: [total_kv_tokens, num_kv_heads=1, head_dim]
                k_pe_mqa = k_pe_for_attn.reshape(-1, 1, self.qk_rope_head_dim)
                kv_c_mqa = kv_c_for_attn.reshape(-1, 1, self.kv_lora_rank)

                # k_mqa: [total_kv_tokens, 1, kv_lora_rank + qk_rope_head_dim]
                k_mqa = torch.cat([kv_c_mqa, k_pe_mqa], dim=-1)
                # v_mqa: [total_kv_tokens, 1, kv_lora_rank] (Value is the compressed latent vector)
                v_mqa = kv_c_mqa

                # Perform MQA using flash_attn_varlen_func
                # Output shape: [total_q_tokens, num_heads, kv_lora_rank] (V dim = kv_lora_rank)
                attn_output_mqa = flash_attn_varlen_func(
                    ...
                    softmax_scale=self.softmax_scale, # Use precomputed scale
                    causal=True, # Causal mask needed
                )
                # attn_output_mqa shape: [bsz * q_len, num_heads, kv_lora_rank]

                # Apply final projection (VLLM's _v_up_proj_and_o_proj logic)
                # Reshape to [bsz*q_len, num_heads, kv_lora_rank] -> [num_heads, bsz*q_len, kv_lora_rank]
                ...
                projected_output = projected_output.transpose(0, 1).reshape(-1, self.num_heads * self.v_head_dim)
                ...
                kv_b_out = self.kv_b_proj(kv_c_for_attn.view(-1, self.kv_lora_rank)) # TE Linear returns tensor
                # Reshape output: [bsz, kv_seq_len, num_heads, qk_nope + v_dim]
                kv_b_out = kv_b_out.view(bsz, kv_seq_len, self.num_heads, self.qk_nope_head_dim + self.v_head_dim)
                # Split into k_nope and v
                # k_nope: [bsz, kv_seq_len, num_heads, qk_nope_head_dim]
                # v: [bsz, kv_seq_len, num_heads, v_head_dim]
                k_nope, v = torch.split(kv_b_out, [self.qk_nope_head_dim, self.v_head_dim], dim=-1)

                # Combine k_nope and k_pe_for_attn (potentially cached)
                # k_pe_for_attn: [bsz, kv_seq_len, qk_rope_head_dim] -> expand for heads
                k_pe_expanded = k_pe_for_attn.unsqueeze(2).expand(-1, -1, self.num_heads, -1)
                # k: [bsz, kv_seq_len, num_heads, q_head_dim]
                k = torch.cat([k_nope, k_pe_expanded], dim=-1)

                # Reshape K and V for FlashAttention: [total_kv_tokens, num_heads, head_dim]
                ...
                if self.v_head_dim < self.q_head_dim:
                    ...
                    v_padded = v # Should be equal if q_head_dim == qk_nope + qk_rope

                # Perform MHA using flash_attn_varlen_func
                # Output shape: [total_q_tokens, num_heads, q_head_dim] (V dim = padded V dim)
                attn_output_padded = flash_attn_varlen_func(
                    q=query_states, # Shape: [total_q_tokens, num_heads, q_head_dim]
                    k=k,            # Shape: [total_kv_tokens, num_heads, q_head_dim]
                    v=v_padded,     # Shape: [total_kv_tokens, num_heads, q_head_dim]
                    cu_seqlens_q=cu_seqlens_q,
                    cu_seqlens_k=cu_seqlens_k,
                    max_seqlen_q=max_seqlen_q,
                    max_seqlen_k=max_seqlen_k,
                ...
                attn_output_proj_input = attn_output_unpadded.reshape(-1, self.num_heads * self.v_head_dim)

        else:
            # --- Fallback to torch.nn.functional.scaled_dot_product_attention ---
            # This path won't implement the specific MLA projections (MQA decode) easily.
            # It will perform standard MHA based on the projected Q, K, V derived from combined states.
            # This serves as a functional fallback if FlashAttention is not available.

            # Project kv_c_for_attn to get k_nope and v (same as prefill path above)
            kv_b_out = self.kv_b_proj(kv_c_for_attn.view(-1, self.kv_lora_rank)) # TE Linear returns tensor
            kv_b_out = kv_b_out.view(bsz, kv_seq_len, self.num_heads, self.qk_nope_head_dim + self.v_head_dim)
            k_nope, v = torch.split(kv_b_out, [self.qk_nope_head_dim, self.v_head_dim], dim=-1)
            ...
            # Combine k_nope and k_pe_for_attn (potentially cached)
            k_pe_expanded = k_pe_for_attn.unsqueeze(2).expand(-1, -1, self.num_heads, -1)
            k = torch.cat([k_nope, k_pe_expanded], dim=-1) # [bsz, kv_seq_len, num_heads, q_head_dim]

            # Reshape Q, K, V for scaled_dot_product_attention: [bsz, num_heads, seq_len, head_dim]
            query_states_sdpa = query_states.reshape(bsz, q_len, self.num_heads, self.q_head_dim).transpose(1, 2)
            key_states_sdpa = k.transpose(1, 2)
            value_states_sdpa = v.transpose(1, 2)

            # scaled_dot_product_attention expects attention_mask: [bsz, num_heads, q_len, k_len]
            ...
            attn_output_sdpa = scaled_dot_product_attention(
                ...
                key_states_sdpa,
                value_states_sdpa,
                attn_mask=None, # Let SDPA handle causal mask based on is_causal=True
                dropout_p=self.dropout_p if self.training else 0.0,
                is_causal=True # Enable causal masking
            ) # Output: [bsz, num_heads, q_len, v_head_dim]

            # Reshape output: [bsz, q_len, num_heads * v_head_dim]
            attn_output_proj_input = attn_output_sdpa.transpose(1, 2).contiguous().view(bsz, q_len, -1)


        # --- Final Output Projection ---
        # attn_output_proj_input shape: [bsz * q_len, num_heads * v_head_dim] or [bsz, q_len, num_heads * v_head_dim]
        attn_output = self.o_proj(attn_output_proj_input) # TE Linear returns tensor
        # Reshape back to [bsz, q_len, hidden_size] if necessary (should be correct already)
        if attn_output.shape[0] != bsz or attn_output.shape[1] != q_len:
             attn_output = attn_output.view(bsz, q_len, self.hidden_size)


        return attn_output, None, present_key_value # Return None for attn_weights
