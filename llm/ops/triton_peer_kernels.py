...
try:
    ...
    HAS_TRITON = False
    print(
        "Triton not found. PEER expert selection will use slower PyTorch implementation."
    )
...
if HAS_TRITON:
    ...
    @triton.jit
    def get_multi_dim_indices(
        expert_idx, dim_sizes_ptr, num_dims: tl.constexpr, stride_ds
    ):
        ...
        for d in range(num_dims - 1, -1, -1):
            ...
            indices = tl.where(
                d == tl.arange(0, num_dims), current_expert_idx % dim_size, indices
            )
    ...
    @triton.autotune(
        configs=[
            triton.Config(
                {"BLOCK_SIZE_T": 64, "BLOCK_SIZE_E": 128, "num_warps": 4}, num_stages=2
            ),
            triton.Config(
                {"BLOCK_SIZE_T": 128, "BLOCK_SIZE_E": 64, "num_warps": 4}, num_stages=2
            ),
            triton.Config(
                {"BLOCK_SIZE_T": 32, "BLOCK_SIZE_E": 256, "num_warps": 8}, num_stages=2
            ),
            triton.Config(
                {"BLOCK_SIZE_T": 256, "BLOCK_SIZE_E": 32, "num_warps": 8}, num_stages=2
            ),
    ...
    def peer_selection_fwd_kernel(
        ...
        stride_do,
        # Normalization
        norm_query: tl.constexpr,
        norm_keys: tl.constexpr,
        # Meta
        ...
        q_ptrs = (
            queries_ptr
            + token_offs[:, None] * stride_qt
            + tl.arange(0, query_dim)[None, :] * stride_qd
        )
        query_block_orig_dtype = tl.load(q_ptrs, mask=token_mask[:, None], other=0.0)
        query_block_fp32 = query_block_orig_dtype.to(
            tl.float32
        )  # Keep fp32 for normalization

        # --- Normalize query block (in FP32) ---
        if norm_query:
            q_norm = tl.sqrt(
                tl.sum(query_block_fp32 * query_block_fp32, axis=1, keep_dims=True)
            )
        ...
        # Convert back to original dtype if normalization was done,
        # as sub-query chunks will be loaded in original dtype later.
        # Keep the FP32 version for dot products if needed.
        # query_block_normed_orig_dtype = query_block_normed_fp32.to(queries_ptr.dtype.element_ty)
        ...
        for expert_start_idx in range(0, num_experts, BLOCK_SIZE_E):
            ...
            current_expert_scores = tl.zeros(
                (BLOCK_SIZE_T, BLOCK_SIZE_E), dtype=tl.float32
            )
            ...
            for d_idx in range(num_dims):
                ...
                sub_key_block_orig_dtype = tl.load(
                    sk_ptrs, mask=expert_mask[:, None], other=0.0
                )
                sub_key_block_fp32 = sub_key_block_orig_dtype.to(
                    tl.float32
                )  # Keep fp32 for normalization
                ...
                if norm_keys:
                    sk_norm = tl.sqrt(
                        tl.sum(
                            sub_key_block_fp32 * sub_key_block_fp32,
                            axis=1,
                            keep_dims=True,
                        )
                    )
                ...
                # query_chunk_normed_fp32 = query_block_normed_fp32[:, q_chunk_offs] # Use the already normalized full query block
                query_chunk_fp32 = query_block_normed_fp32[:, q_chunk_offs] # Select the chunk from the potentially normalized FP32 query block

                # --- Matmul (No explicit FP8 casting here - rely on ambient precision from fp8_autocast) ---
                # Inputs (query_chunk_fp32, sub_key_block_normed_fp32) are FP32
                ...
                dim_scores = tl.dot(
                    query_chunk_fp32, # Use the selected FP32 query chunk
                    tl.trans(sub_key_block_normed_fp32),
                    out_dtype=tl.float32,
                    allow_tf32=False, # Ensure TF32 is off for precision if needed
                )
            ...
            expert_indices_block = expert_offs[None, :].to(
                tl.int32
            )  # Shape [1, BLOCK_SIZE_E]
            expert_indices_block_expanded = tl.broadcast_to(
                expert_indices_block, (BLOCK_SIZE_T, BLOCK_SIZE_E)
            )
            combined_indices = tl.cat(
                (top_k_indices, expert_indices_block_expanded), axis=1
            )  # Shape [BLOCK_SIZE_T, top_k + BLOCK_SIZE_E]
            ...
            sorted_scores, sorted_relative_indices = tl.sort(
                combined_scores, axis=1, descending=True
            )
            ...
            top_k_relative_indices = sorted_relative_indices[
                :, :top_k
            ...
            token_idx_offsets = (
                tl.arange(0, BLOCK_SIZE_T)[:, None] * combined_indices.shape[1]
            )
        ...
        oi_ptrs = (
            output_indices_ptr
            + token_offs[:, None] * stride_oi_t
            + tl.arange(0, top_k)[None, :] * stride_oi_k
        )
        os_ptrs = (
            output_scores_ptr
            + token_offs[:, None] * stride_os_t
            + tl.arange(0, top_k)[None, :] * stride_os_k
        )
        tl.store(oi_ptrs, top_k_indices, mask=token_mask[:, None])
        tl.store(
            os_ptrs,
            top_k_scores.to(output_scores_ptr.dtype.element_ty),
    ...
    def peer_selection_bwd_kernel(
        ...
        stride_oi_k,
        stride_gs_t,
        stride_gs_k,  # grad_scores strides
        stride_gq_t,
        stride_gq_d,
        stride_gsk_t,
        stride_gsk_d,  # grad inputs strides
        stride_ds,
        stride_do,
        # Normalization
        norm_query: tl.constexpr,
        ...
        top_indices = tl.load(
            oi_ptrs, mask=token_mask[:, None], other=-1
        )  # [BLOCK_SIZE_T, top_k]
        gs_ptrs = (
            grad_scores_ptr
            + token_offs[:, None] * stride_gs_t
            + tl.arange(0, top_k)[None, :] * stride_gs_k
        )
        ...
        grad_scores_fp32 = grad_scores_orig_dtype.to(
            tl.float32
        )  # [BLOCK_SIZE_T, top_k]
        ...
        for k_idx in range(top_k):
            ...
            for d_idx in range(num_dims):
                ...
                grad_q_chunk_fp32 = (
                    current_grad_scores[:, None] * sub_key_block_normed_fp32
                )  # [BLOCK_SIZE_T, sub_query_dim]
                ...
                # grad(x_norm) = grad(x / norm(x)) = (grad_normed * norm(x) - x_norm * sum(grad_normed * x_norm)) / norm(x)
                if norm_query:
                    q_chunk_normed_fp32 = query_block_normed_fp32[:, q_chunk_offs] # Normalized chunk
                    grad_q_normed_fp32 = grad_q_chunk_fp32 # Gradient w.r.t. normalized chunk
                    # Use the precomputed full query norm (q_norm)
                    norm_val = q_norm + 1e-6
                    sum_grad_q_times_q = tl.sum(grad_q_normed_fp32 * q_chunk_normed_fp32, axis=1, keep_dims=True)
                    grad_q_unnormed_fp32 = (grad_q_normed_fp32 - q_chunk_normed_fp32 * sum_grad_q_times_q) / norm_val

                    # Create a mask for the current chunk dimensions
                    dim_mask = (tl.arange(0, query_dim) >= (d_idx * sub_query_dim)) & (tl.arange(0, query_dim) < ((d_idx + 1) * sub_query_dim))
                    # Accumulate gradient only for the current chunk dimensions
                    grad_queries_acc = tl.where(
                        expert_mask[:, None] & token_mask[:, None] & dim_mask[None, :],
                        grad_queries_acc + grad_q_unnormed_fp32,
                        grad_queries_acc,
                    )
                else:
                    # Create a mask for the current chunk dimensions
                    ...
                    grad_queries_acc = tl.where(
                        expert_mask[:, None] & token_mask[:, None] & dim_mask[None, :],
                        grad_queries_acc + grad_q_chunk_fp32,
                        grad_queries_acc,
                    )

                # --- Calculate Gradient w.r.t. Sub-Key Chunk (FP32 Accumulation with Atomics) ---
                # grad_sk = sum_tokens(grad_score * query_chunk_normed)
                query_chunk_normed_fp32 = query_block_normed_fp32[
                    :, q_chunk_offs
                ]  # [BLOCK_SIZE_T, sub_query_dim]
                # Calculate per-token gradient contribution (in FP32)
                grad_sk_normed_per_token_fp32 = (
                    current_grad_scores[:, None] * query_chunk_normed_fp32
                )  # [BLOCK_SIZE_T, sub_query_dim]

                # Handle key normalization gradient (in FP32)
                if norm_keys:
                    # grad(x_norm) = grad(x / norm(x)) = (grad_normed * norm(x) - x_norm * sum(grad_normed * x_norm)) / norm(x)
                    grad_sk_normed_fp32 = grad_sk_normed_per_token_fp32 # Gradient w.r.t. normalized key
                    norm_val = sk_norm + 1e-6
                    sum_grad_sk_times_sk = tl.sum(grad_sk_normed_fp32 * sub_key_block_normed_fp32, axis=1, keep_dims=True)
                    grad_sk_unnormed_fp32 = (grad_sk_normed_fp32 - sub_key_block_normed_fp32 * sum_grad_sk_times_sk) / norm_val
                    grad_sk_to_add_fp32 = grad_sk_unnormed_fp32
                else:
                ...
                grad_sk_to_add_output_dtype = grad_sk_to_add_fp32.to(
                    grad_sub_keys_cat_ptr.dtype.element_ty
                )
                tl.atomic_add(
                    grad_sk_ptrs,
                    grad_sk_to_add_output_dtype,
                    mask=expert_mask[:, None] & token_mask[:, None],
                )

        # --- Store accumulated query gradients (cast back to original dtype) ---
        grad_q_ptrs = (
            grad_queries_ptr
            + token_offs[:, None] * stride_gq_t
            + tl.arange(0, query_dim)[None, :] * stride_gq_d
        )
        tl.store(
            grad_q_ptrs,
            grad_queries_acc.to(grad_queries_ptr.dtype.element_ty),
            mask=token_mask[:, None],
        )
    ...
    class PEERSelectionFunction(Function):
        @staticmethod
        def forward(
            ctx, queries, sub_keys_list, product_key_dim, top_k, norm_query, norm_keys
        ):
            if not HAS_TRITON:
            ...
            sub_keys_cat = torch.cat(
                [
                    sk.contiguous().to(queries.device, dtype=queries.dtype)
                    for sk in sub_keys_list
                ],
                dim=0,
            )
            ...
            # noqa: E731 - lambda used for consistency with triton tutorials
            # Function to define grid for triton kernel
            def get_grid(META):
                return (triton.cdiv(num_tokens, META["BLOCK_SIZE_T"]),)
            grid = get_grid
            ...
            peer_selection_fwd_kernel[grid](
                ...
                num_dims,
                sub_query_dim,
                queries_flat.stride(0),
                queries_flat.stride(1),
                sub_keys_cat.stride(0),
                sub_keys_cat.stride(1),
                output_indices.stride(0),
                output_indices.stride(1),
                output_scores.stride(0),
                output_scores.stride(1),
                dim_sizes.stride(0),
                dim_offsets.stride(0),
                norm_query,
                norm_keys,
            ...
            # Reshape outputs
            output_indices_reshaped = output_indices.view(
                batch_size, seq_len, num_heads, top_k
            )
            output_scores_reshaped = output_scores.view(
                batch_size, seq_len, num_heads, top_k
            )
        ...
        def backward(ctx, grad_indices, grad_scores):
            ...
            peer_selection_bwd_kernel[grid](
                ...
                dim_offsets,
                grad_queries,
                grad_sub_keys_cat,  # Output grads match input dtypes
                num_tokens,
                query_dim,
                num_experts,
                top_k,
                ...
                output_indices.stride(0),
                output_indices.stride(1),
                grad_scores_flat.stride(0),
                grad_scores_flat.stride(1),
                grad_queries.stride(0),
                grad_queries.stride(1),
                grad_sub_keys_cat.stride(0),
                grad_sub_keys_cat.stride(1),
                dim_sizes.stride(0),
                dim_offsets.stride(0),
                norm_query,
                norm_keys,
            ...
            for dim_size in dim_sizes_list:
                ...
                grad_slice = grad_sub_keys_cat[
                    current_offset:current_offset + dim_size
                ]
    ...
    def peer_selection_triton(
        queries, sub_keys_list, product_key_dim, top_k, norm_query, norm_keys
    ):
        # Ensure sub_keys_list contains tensors, not parameters, for autograd function
        sub_key_tensors = [
            p.data if isinstance(p, torch.nn.Parameter) else p for p in sub_keys_list
        ]
        return PEERSelectionFunction.apply(
            queries, sub_key_tensors, product_key_dim, top_k, norm_query, norm_keys
        )
